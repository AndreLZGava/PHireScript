...
/**
 * @extends AbstractContext<GroupedExpressionNode>
 */
class GroupedExpressionContext extends AbstractContext
{
    private readonly array $resolvers;

    public function __construct(GroupedExpressionNode $node)
    {
    ...
    public function handle(Token $token, ParseContext $parseContext): ?Node
    {
        ...
        throw new CompileException(
            $token->value . ' is not supported in grouped expression context!',
            $token->line,
            $token->column,
        );
    }
    ...
    {
        $inner = !empty($this->children) ? end($this->children) : null;
        if ($inner !== null) {
            $this->node->inner = $inner;
        }
