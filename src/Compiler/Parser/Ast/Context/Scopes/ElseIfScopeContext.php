...
/**
 * @extends AbstractContext<ElseIfScopeNode>
 */
class ElseIfScopeContext extends AbstractContext
{
    private readonly array $resolvers;

    public function __construct(ElseIfScopeNode $node)
    {
    ...
    public function handle(Token $token, ParseContext $parseContext): ?Node
    {
        ...
        throw new CompileException(
            $token->value . ' is not supported in elseif scope definition context!',
            $token->line,
            $token->column,
        );
    }
    ...
    public function afterClose(Token $token, ParseContext $parseContext): void
    {
        if ($token->isClosingCurlyBracket()) {
            $parseContext->contextManager->exit(); // exit ElseIfContext
            $next = $parseContext->tokenManager->getNextTokenAfterCurrent();
            if ($next->value !== 'else' && $next->value !== 'elseif') {
                $parseContext->contextManager->exit(); // exit IfContext
            }
        }
    }
